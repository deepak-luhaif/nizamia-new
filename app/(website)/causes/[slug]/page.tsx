...
const Cause = () => {
  ...
  const cause = causesData.find((cause) => cause.slug === slug);
    ...
    <>
      <div className=' mt-20'>
        <div className='relative'>
          ...
          <div className='absolute top-1/2 left-[100px] transform '>
            <h1 className='text-white font-semibold text-3xl'>{cause.tag}</h1>
            <p className='text-white text-lg'> {cause.title}</p>
          </div>
        </div>
      ...
      <div className='container-box'>
        <div className='flex flex-col gap-5'>
          <div className='text-center'>
            ...
            <h2 className='font-bold text-3xl '>{cause.title}</h2>
          </div>
          <div>
      ...
      <div className='container-box'>
        <h4 className='text-blueColor text-3xl font-semibold'>
          Related Latest causes
        </h4>
        <div className='grid lg:grid-cols-3 grid-cols-1 gap-5 mt-10'>
          {/* Display the latest causes */}
          {causesData
            .filter((item) => item.slug !== slug) // Exclude the current cause
            .slice(0, 3) // Limit to the latest 3 causes
            .map((cause) => (
              <CauseCard
                key={cause?.slug}
                img={cause?.img}
